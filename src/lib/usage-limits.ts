...
export async function checkAIGenerationUsageLimit(userId: string): Promise<ValidationResult & { canGenerate?: boolean; currentCount?: number; limit?: number | typeof Infinity }> {
    try {
        ...
        // --- Use getAIGenerationUsageForMonth ---
...
export async function getUserUsage(userId: string) {
   try {
    ...
    const [
      ...
      supabaseHelpers.getAIGenerationUsageForMonth(userId, new Date()),
    ...
    const docLimit = isPro ? USAGE_LIMITS.PRO_DOCUMENTS : USAGE_LIMITS.FREE_DOCUMENTS;

    return {
      plan,
      notes: { used: notesCount, limit: noteLimit, remaining: isPro ? Infinity : Math.max(0, noteLimit - notesCount) },
      quizzes: { used: quizzesCount, limit: quizLimit, remaining: isPro ? Infinity : Math.max(0, quizLimit - quizzesCount) },
      aiGenerations: { used: aiGenerationCountThisMonth, limit: aiLimit, remaining: isPro ? Infinity : Math.max(0, aiLimit - aiGenerationCountThisMonth) },
      flashcardDecks: { used: decksCount, limit: deckLimit, remaining: isPro ? Infinity : Math.max(0, deckLimit - decksCount) },
      flashcards: { used: flashcardsCount, limit: cardLimit, remaining: isPro ? Infinity : Math.max(0, cardLimit - flashcardsCount) },
      documents: { used: documentsCount, limit: docLimit, remaining: isPro ? Infinity : Math.max(0, docLimit - documentsCount) }
    };
  } catch (error) {
