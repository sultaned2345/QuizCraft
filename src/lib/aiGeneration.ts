...
export async function generatePodcastForDocument(
  ...
  try {
    ...
    // D. Save to Database (FIX: mapped to 'recordings' table as 'podcast' model does not exist)
    return await prisma.recordings.create({
      data: {
        user_id: userId,
        title: `Podcast: ${title}`,
        // We store the script in transcript
        transcript: JSON.stringify(script), 
        // We store the audio URL in the suggestions JSON field since there's no audio_url column
...
// FIX: Point to the correct structured insights generator
