...
async function readMultipartOrText(request: NextRequest): Promise<{ text: string; sourceType: "text" | "file"; }> {
  ...
  if (contentType.includes("multipart/form-data")) {
    ...
    console.warn("generate-quiz received multipart/form-data, expected text/plain. Attempting to read 'text' field.");
...
function buildPrompt({ text, numQuestions, difficulty, questionType }: { text: string; numQuestions: number; difficulty: Difficulty; questionType: QuestionTypeOption }) {
    const questionTypes = questionType === 'MIXED'
      ? 'MULTIPLE_CHOICE, TRUE_FALSE, and FILL_IN_THE_BLANK' // Removed MATCHING temporarily
      : questionType;
    ...
    // Removed MATCHING type from example structure for simplicity/reliability
...
async function callGeminiForQuiz({ text, numQuestions, difficulty, questionType }: { text: string; numQuestions: number; difficulty: Difficulty, questionType: QuestionTypeOption }): Promise<{ title: string; questions: Question[] }> {
  ...
  try {
    ...
    console.error("Failed to parse Gemini response:", cleanedContent.substring(0, 500)); // Log snippet on error
  ...
  for (let i = 0; i < parsed.questions.length; i++) {
    ...
    if (!q || !q.question_text || !q.question_type || !q.correct_answer) {
        ...
        throw new Error(`Question ${i + 1} is missing required fields (question_text, question_type, correct_answer).`);
    }
    if (q.question_type === 'MULTIPLE_CHOICE' && (!Array.isArray(q.options) || q.options.length !== 4)) { // Expect exactly 4 options now
        console.error(`Invalid MULTIPLE_CHOICE options at index ${i}:`, q.options);
    ...
    if (q.question_type === 'MULTIPLE_CHOICE' && !q.options.includes(q.correct_answer)) {
        console.error(`Correct answer mismatch at index ${i}: Answer='${q.correct_answer}', Options=${JSON.stringify(q.options)}`);
        throw new Error(`Question ${i + 1} (MULTIPLE_CHOICE): correct_answer ('${q.correct_answer}') must exactly match one of the options.`);
    }
     if (q.question_type === 'TRUE_FALSE' && !["True", "False"].includes(q.correct_answer)) {
        console.error(`Invalid TRUE_FALSE answer at index ${i}: Answer='${q.correct_answer}'`);
        throw new Error(`Question ${i + 1} (TRUE_FALSE): correct_answer must be 'True' or 'False'.`);
    }
    // Add validation for FILL_IN_THE_BLANK if needed (e.g., check for '____')
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!text) {
     ...
     if (text.length < 100) { // Add length check here too
    ...
    console.log("----- Data going to Prisma START -----\n", JSON.stringify(quiz, null, 2), "\n----- Data going to Prisma END -----");
    // --- END DEBUGGING ---
    ...
    const saved = await prisma.quiz.create({
      data: {
       ...
       select: {
           ...
           questions: { // Include generated questions in response
               select: {
                   id: true,
                   question_text: true,
                   question_type: true,
    ...
    if (message.includes("Content-Type") || message.includes("No input text provided") || message.includes("too short")) status = 400;
