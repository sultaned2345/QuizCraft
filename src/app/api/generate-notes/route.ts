...
// Updated callAIToGenerateNotes function WITH RAW RESPONSE LOGGING
async function callAIToGenerateNotes(text: string): Promise<Array<{ title: string; content: string; }>> {
  ...
  try {
    ...
    const rawContent = response.text(); // Get the raw text response
    ...
    if (!rawContent) { // Check the raw content
    ...
    try {
        ...
        console.error(`AI Error: Failed to parse JSON response from ${AI_MODEL_NAME}. Raw response snippet:`, rawContent.substring(0, 500));
        // Attempt to extract JSON manually if surrounded by markdown etc.
        ...
        if (jsonMatch) {
            try {
                ...
                parsed = JSON.parse(jsonMatch[0]);
                 ...
                 throw new Error(`Invalid JSON structure returned by AI, even after fallback parsing.`);
            }
        } else {
            throw new Error(`Invalid JSON structure returned by AI. No JSON object found.`);
        }
    }
    ...
    if (parsed.notes.length === 0) {
        ...
        throw new Error("AI failed to generate the notes.");
    }

    const validNotes = parsed.notes.filter((note: any) => {
        const isValid = note && typeof note.title === 'string' && note.title.trim().length > 0 &&
                        typeof note.content === 'string' && note.content.trim().length > 10; // Check for minimal content length
        if (!isValid) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error.message?.startsWith("Failed to generate notes") || error.message?.includes("AI generated invalid note content") || error.message?.includes("Invalid JSON structure")) {
