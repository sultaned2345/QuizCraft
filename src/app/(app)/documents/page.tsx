...
export default function LibraryPage() {
  ...
  const handleDelete = async (e: React.MouseEvent, type: string, id: string) => {
    ...
    try {
      // Map frontend type to API endpoint structure if needed, 
      // or use a unified delete endpoint. For now, we assume a unified one or specific ones.
      // Simplest: specific endpoints per type.
      let endpoint = '';
      if (type === 'document') endpoint = `/api/documents/${id}`;
      else if (type === 'quiz') endpoint = `/api/quiz/${id}`;
      else if (type === 'note') endpoint = `/api/notes/${id}`;
      else if (type === 'deck') endpoint = `/api/decks/${id}`;

      const res = await fetch(endpoint, { method: 'DELETE' });
      if (!res.ok) throw new Error('Failed to delete');

      toast({ title: 'Item deleted', description: 'The content has been removed from your library.' });
      mutate(); // Refresh list
    } catch (err) {
              ...
              <div className="px-4 pb-4 flex-1 flex flex-col pointer-events-none">
                ...
                <div className="mt-auto pt-4 flex items-center justify-between text-xs text-muted-foreground">
                  <span className="capitalize bg-muted px-2 py-0.5 rounded-md font-medium">
                    {item.type === 'deck' ? 'Flashcards' : item.type}
                  </span>
