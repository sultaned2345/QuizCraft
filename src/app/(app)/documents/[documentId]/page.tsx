...
import { 
  FileText, BrainCircuit, Layers, Mic, PanelRightClose, PanelRightOpen,
...
export default function StudyWorkspacePage() {
  ...
  // --- Auto-Polling for Background Generation ---
  // If the document is still "processing", check back every 3 seconds
  useEffect(() => {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] -m-4 md:-m-8 overflow-hidden flex flex-col bg-background">
      ...
      <ResizablePanelGroup direction="horizontal" className="flex-1 h-full">
        ...
        {/* LEFT PANEL: CHAT */}
        <ResizablePanel defaultSize={35} minSize={25} maxSize={50} className="flex flex-col border-r border-border/40 bg-card/30 backdrop-blur-sm">
          <div className="h-14 flex items-center justify-between px-4 border-b border-border/40 bg-background/50">
             ...
             <Button variant="ghost" size="sm" onClick={() => setIsContentOpen(!isContentOpen)}>
            ...
            <div className="h-14 flex items-center justify-between px-4 border-b border-border/40 bg-background/80 backdrop-blur-md">
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full h-full flex items-center">
                <TabsList className="h-9 bg-muted/50 p-1 rounded-lg">
                  <TabsTrigger value="document"><FileText className="w-3.5 h-3.5 mr-2" /> PDF</TabsTrigger>
                  <TabsTrigger value="notes"><PenTool className="w-3.5 h-3.5 mr-2" /> Notes</TabsTrigger>
                  <TabsTrigger value="quiz"><BrainCircuit className="w-3.5 h-3.5 mr-2" /> Quiz</TabsTrigger>
                  <TabsTrigger value="flashcards"><Layers className="w-3.5 h-3.5 mr-2" /> Cards</TabsTrigger>
                  <TabsTrigger value="audio"><Mic className="w-3.5 h-3.5 mr-2" /> Audio</TabsTrigger>
                </TabsList>
              </Tabs>
            </div>
              ...
              <Tabs value={activeTab} className="h-full w-full">
                ...
                <TabsContent value="notes" className="h-full m-0 p-0">
                  {isLoading ? (
                    ...
                    <NoteEditor noteId={studySet.note.id} initialContent={studySet.note.content} initialTitle={studySet.note.title} />
                  ) : (
                    <div className="h-full flex flex-col items-center justify-center space-y-4 p-8 text-center">
                        <div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center">
                           <PenTool className="w-8 h-8 text-primary" />
                        </div>
                        {isTabGenerating('note') ? (
                           <>
                             <h3 className="text-xl font-medium animate-pulse">Generating Notes...</h3>
                             <p className="text-muted-foreground">AI is summarizing your document.</p>
                             <Loader2 className="w-6 h-6 animate-spin text-primary mt-2" />
                           </>
                        ) : (
                           <>
                             <h3 className="text-xl font-medium">No Notes Yet</h3>
                             <Button onClick={() => generate('note')}>
                               <Sparkles className="mr-2 w-4 h-4" /> Generate Notes
                             </Button>
                           </>
                        )}
                    </div>
                      ...
                      <div className="max-w-md mx-auto mt-10 space-y-6">
                         <div className="p-8 rounded-3xl border bg-card shadow-lg text-center space-y-4">
                            ...
                            <div>
                               ...
                               <p className="text-muted-foreground">{studySet.quiz.questions?.length || 0} Questions</p>
                            </div>
                         ...
                         <div className="text-center">
                             <Button variant="link" onClick={() => generate('quiz')} disabled={isTabGenerating('quiz')}>Regenerate</Button>
                         </div>
                      </div>
                   ) : (
                      <div className="h-full flex flex-col items-center justify-center space-y-4 p-8 text-center">
                          ...
                          {isTabGenerating('quiz') ? (
                             <>
                               <h3 className="text-xl font-medium animate-pulse">Creating Quiz...</h3>
                               <p className="text-muted-foreground">AI is crafting questions for you.</p>
                               <Loader2 className="w-6 h-6 animate-spin text-primary mt-2" />
                             </>
                          ) : (
                             <>
                               <h3 className="text-xl font-medium">Test Your Knowledge</h3>
                               <Button onClick={() => generate('quiz')}>
                                   <Sparkles className="mr-2 w-4 h-4" /> Generate Quiz
                               </Button>
                             </>
                          )}
                      </div>
                      ...
                      <div className="max-w-md mx-auto mt-10 space-y-6">
                         <div className="p-8 rounded-3xl border bg-card shadow-lg text-center space-y-4">
                            ...
                            <div>
                               ...
                               <p className="text-muted-foreground">{studySet.deck.flashcards?.length || 0} Cards</p>
                            </div>
                         ...
                         <div className="text-center">
                             <Button variant="link" onClick={() => generate('flashcards')} disabled={isTabGenerating('flashcards')}>Regenerate</Button>
                         </div>
                      </div>
                   ) : (
                      <div className="h-full flex flex-col items-center justify-center space-y-4 p-8 text-center">
                          ...
                          {isTabGenerating('flashcards') ? (
                             <>
                               <h3 className="text-xl font-medium animate-pulse">Drafting Flashcards...</h3>
                               <p className="text-muted-foreground">AI is extracting key concepts.</p>
                               <Loader2 className="w-6 h-6 animate-spin text-primary mt-2" />
                             </>
                          ) : (
                             <>
                               <h3 className="text-xl font-medium">Master the Details</h3>
                               <Button onClick={() => generate('flashcards')}>
                                   <Sparkles className="mr-2 w-4 h-4" /> Generate Flashcards
                               </Button>
                             </>
                          )}
                      </div>
                   ...
                   {isLoading ? (
                      ...
                      <div className="max-w-2xl mx-auto space-y-8 mt-10">
                         <div className="text-center space-y-2">
                            ...
                            <p className="text-muted-foreground">Listen to an AI-generated discussion.</p>
                         </div>
                         {docData && (
                           <PodcastPlayer 
