...
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from "@/components/ui/accordion"; // Keep accordion import for highlights
...
type GradedEssayListItem = Pick<
  GradedEssay,
  'id' | 'essay_title' | 'score' | 'graded_at'
>;
...
const rubricPresets = {
  general: {
    name: 'General',
    rubric:
      'Evaluate based on standard academic criteria: Clarity (Is the point clear?), Argument (Is the logic sound?), and Grammar (Are there errors?).',
  },
  persuasive: {
    name: 'Persuasive',
    rubric:
      'Evaluate this as a persuasive essay. Focus on: (1) The strength and clarity of the thesis statement, (2) The quality and relevance of supporting evidence, (3) The effectiveness of the counter-argument and rebuttal, and (4) The overall rhetorical impact.',
  },
  admission: {
    name: 'Admission',
    rubric:
      'Evaluate this as a college admission essay. Focus on: (1) A compelling personal narrative, (2) A strong and unique authorial voice, (3) Clarity of thought and structure, and (4) Flawless grammar and style.',
  },
};
...
export default function EssayGraderPage() {
  ...
  const [gradedEssay, setGradedEssay] = useState<GradeEssayResponseData | null>(
    null
  );
  ...
  const {
    ...
    (url: string) =>
      fetcher(url, { headers: { Authorization: `Bearer ${session!.access_token}` } }),
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      const isValidExt = ['.pdf', '.txt'].some((ext) =>
  ...
  const handleViewHistoryItem = async (essayId: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Displaying feedback for "${
          resultData.essay_title || 'graded essay'
        }".`,
      });
    } catch (err: any) {
      setError(
        err.message || 'An unexpected error occurred while fetching history.'
      );
      toast({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (inputMode === 'file' && selectedFile) {
        ...
        const body: { essayText: string; rubricText?: string } = {
      ...
      if (!response.ok || !result.success || !result.data) {
        ...
        if (result.error?.includes('too short')) {
          throw new Error(
            'The essay content is too short (minimum 50 characters required). Please provide more text.'
          );
        }
      ...
      toast({
        ...
        description: 'Your essay feedback is ready.',
      });
      ...
      const errorMessage =
        err.message || 'An unexpected error occurred during grading.';
  ...
  const renderHighlightedEssay = (
    ...
    const categories: ('clarity' | 'argument' | 'grammar')[] = [
      'clarity',
      'argument',
      'grammar',
    ];
  ...
  // --- REFACTORED: renderFeedback function ---
      ...
      <div className="space-y-6">
        {fb.summary && (
          <div className="mb-4 p-4 bg-muted/50 rounded-lg">
            <h4 className="font-semibold text-lg mb-2">Overall Summary</h4>
        ...
        {/* Removed the main Accordion wrapper, replaced with a div */}
        <div className="space-y-6">
          {categories.map((key) => {
            const data = fb[key];
            if (!data) return null;
            ...
            if (typeof data === 'object' && data.summary) {
              return (
                <div key={key} className="border-b pb-4 last:border-b-0">
                  <h4 className="flex items-center gap-2 text-lg font-semibold capitalize mb-3">
                    {getFeedbackIcon(key)}
                    <span>{key}</span>
                  </h4>
                    ...
                    {data.highlights && data.highlights.length > 0 && (
                      <Accordion type="single" collapsible className="w-full">
                        <AccordionItem value="item-1" className="border-b-0">
                          <AccordionTrigger className="text-xs pt-0">
                          ...
                          <AccordionContent>
                            <ul className="space-y-2 pt-2">
                              {data.highlights.map((h, i) => (
                                <li
                                  key={i}
                                  className="text-xs border-l-2 pl-3 py-1 border-border/50"
                                >
                                  <blockquote className="font-mono text-foreground p-2 bg-muted rounded">
                                    "{h.text}"
                                  </blockquote>
  ...
  const isOverLimit =
    ...
    (aiUsage.currentCount ?? 0) >= (aiUsage.limit ?? Infinity);
  const isOverTextLimit =
    inputMode === 'text' && (wordCount > WORD_LIMIT || !essayText.trim());
        ...
        <div className="text-sm text-muted-foreground">
          {isUsageLoading ? (
            <span className="flex items-center gap-1">
              <Loader2 className="h-4 w-4 animate-spin" /> Checking AI usage...
            </span>
          ) : usageError ? (
            ...
            aiUsage.isPro ? (
              <span className="font-medium text-primary flex items-center gap-1">
                <Star className="w-4 h-4" /> Pro Plan: Unlimited AI Generations
              </span>
            ) : (
              <span>
                AI Generations:{' '}
                <span className="font-medium text-foreground">
                  {aiUsage.currentCount ?? '?'} / {aiUsage.limit}
                </span>
              </span>
            )
          ) : null}
        ...
        <div className="lg:col-span-1 space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Paste text or upload a file (PDF/TXT, Max 3MB).
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex justify-center mb-4 border border-input rounded-lg p-1 w-min mx-auto bg-background">
                <Button
                  ...
                  className="w-28 h-8 text-xs sm:text-sm"
                >
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Provide specific instructions or a rubric for the AI.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Textarea
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                <Button
                  ...
                  onClick={() => setRubricText(rubricPresets.general.rubric)}
                >
                ...
                <Button
                  ...
                  className="text-xs h-7"
                  onClick={() =>
                    setRubricText(rubricPresets.persuasive.rubric)
                  }
                >
                ...
                <Button
                  ...
                  variant="outline"
                  className="text-xs h-7"
                  onClick={() => setRubricText(rubricPresets.admission.rubric)}
                >
        ...
        <div className="lg:col-span-1">
          <Card className="min-h-[400px] flex flex-col">
            <Tabs
              value={outputTab}
              onValueChange={(value) =>
                setOutputTab(value as 'feedback' | 'history')
              }
                ...
                <CardContent className="flex-1 flex flex-col">
                  {isLoading ? (
                    ...
                    <ScrollArea className="h-full max-h-[60vh] p-1 pr-3">
                      ...
                      {gradedEssay.suggestions &&
                        gradedEssay.suggestions.length > 0 && (
                          <div className="mt-6 pt-6 border-t">
                            <h4 className="font-semibold text-lg mb-2">
                              Suggestions for Improvement
                            </h4>
                            <ul className="list-disc pl-5 space-y-1 text-sm text-muted-foreground">
                              {gradedEssay.suggestions.map((s, i) => (
                                <li key={i}>{s}</li>
                              ))}
                            </ul>
                          </div>
                      ...
                      <div className="space-y-2">
                        {history.map((item) => (
                          <div
                            ...
                            onClick={() =>
                              !isLoading && handleViewHistoryItem(item.id)
                            }
                            tabIndex={0}
                            onKeyDown={(e) =>
                              e.key === 'Enter' &&
                              !isLoading &&
                              handleViewHistoryItem(item.id)
                            }
                          >
                            <div className="text-left">
                              <p className="font-medium text-sm truncate">
                                {item.essay_title || 'Untitled Essay'}
                              </p>
